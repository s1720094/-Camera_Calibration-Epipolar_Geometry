...
def calibrate2D(ref3D, ref2D):
    ...
    Hxz = np.append(np.linalg.lstsq(Axz, b, rcond = None)[0], 1)
    ...
    # TODO : solve for the planar projective transformation using linear least squares
    Hyz = np.append(np.linalg.lstsq(Ayz, b, rcond = None)[0], 1)
...
def gen_correspondences(Hxz, Hyz, corners) :
    ...
    nXZ = np.zeros((corners.shape[0], 3))
    nYZ = np.zeros((corners.shape[0], 3))

    for k in range(corners.shape[0]):
        s = corners[k][2]
        su = corners[k][2] * corners[k][0]
        sv = corners[k][2] * corners[k][1]

        c = np.array([su, sv, s]).T
        c3Dxz = inv(Hxz).dot(c)
        c3Dyz = inv(Hyz).dot(c)

        c3Dxz[0] = c3Dxz[0]/c3Dxz[2]
        c3Dxz[1] = c3Dxz[1]/c3Dxz[2]
        c3Dxz[2] = 1
        nXZ[k] = c3Dxz.T

        c3Dyz[0]= c3Dyz[0]/c3Dyz[2]
        c3Dyz[1]= c3Dyz[1]/c3Dyz[2]
        c3Dyz[2]= 1
        nYZ[k] = c3Dyz.T
    ...
    for k in range(nXZ.shape[0]):
        if(nXZ[k][0] >= 0 and nXZ[k][1] >= 0 and nXZ[k][0] <= 10 and nXZ[k][1] <= 8):
            tcor = Hxz.dot(nXZ[k])
            tcor = tcor/tcor[2]
            corXZ2d.append(tcor[0:2])
            corXZ3d.append(nXZ[k][0:2])

    corXZ2d = np.array(corXZ2d)
    corXZ3d = np.array(corXZ3d)
    tempv = np.zeros((corXZ3d.shape[0],1))
    corXZ3d = np.hstack((corXZ3d[:,:1], tempv, corXZ3d[:,1:]))

    # TODO : project corners on the calibration plane 2 onto the image (YZ-plane)
    corYZ2d = []
    corYZ3d = []
    for k in range(nYZ.shape[0]):
        if ((nYZ[k][0] >= 0)  and (nYZ[k][1] >= 0) and (nYZ[k][0] <= 10 and (nYZ[k][1] <= 8))):
            tcor = Hyz.dot(nYZ[k])
            tcor = tcor/tcor[2]
            corYZ2d.append(tcor[0:2])
            corYZ3d.append(nYZ[k][0:2])

    corYZ2d = np.array(corYZ2d)
    corYZ3d = np.array(corYZ3d)
    tempv = np.zeros((corYZ3d.shape[0],1))
    corYZ3d = np.hstack((tempv,corYZ3d))

    # TODO : locate the nearest detected corners
    ref3D = np.concatenate((corXZ3d, corYZ3d), axis=0)
    ref2D = np.concatenate((corXZ2d, corYZ2d), axis=0)
...
def calibrate3D(ref3D, ref2D) :
    ...
    P = np.append(np.linalg.lstsq(A, b, rcond = None)[0] ,1)
    P = np.reshape(P, (3,4))
...
def main() :
    ...
    parser.add_argument('-i', '--image', type = str, default = 'grid2.jpg',
                        help = 'filename of input image')
    parser.add_argument('-c', '--corners', type = str, default = 'grid2.crn',
                        help = 'filename of corner detection output')
    parser.add_argument('-o', '--output', type = str, default = 'grid2.cam',
